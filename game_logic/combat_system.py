...
class CombatState:
    ...
    它是一个有状态的对象，用于取代旧的“纯函数”方法。

    使用流程:
    1. (Controller) combat = CombatState(attacker, defender, action, ...)
    2. (Controller) log, packet = combat.resolve(log)
    3. (Controller) game_state = _apply_combat_packet(game_state, packet, log)
    4. (Controller) if combat.stage != 'RESOLVED':
    5. (Controller)     mech.pending_combat = combat.to_dict()
    """
    ...
    def __init__(self, attacker_entity, defender_entity, action, target_part_name,
        ...
        # 'INITIAL_ROLL': 初始掷骰
        # 'AWAITING_ATTACK_REROLL': 等待对主攻击的重投
        # 'AWAITING_EFFECT_CHOICE': 等待选择溢出效果
        # 'AWAITING_EFFECT_REROLL': 等待对效果掷骰的重投
        # 'RESOLVED': 战斗已结束
    ...
    def resolve(self, log, chosen_effect=None, rerolled_attack_raw=None, rerolled_defense_raw=None):
        ...
        try:
            ...
            log.append(traceback.format_exc())  # 添加堆栈跟踪
            # 返回一个无效包，防止状态污染
            self.stage = 'RESOLVED'  # 标记为已解决，防止卡死
    ...
    def _resolve_initial_roll(self, log):
        """
        (私有) 执行战斗的第一次掷骰和结算。
        这可能会导致 'RESOLVED', 'AWAITING_ATTACK_REROLL', 或 'AWAITING_EFFECT_CHOICE'。
        """
        # --- 1. 初始化 ---
        log.append(f"> {self.attacker_entity.name} 使用 [{self.action.name}] 攻击 {self.defender_entity.name}。")
        result_packet = self._create_empty_packet('miss')  # 默认 'miss'
        dice_roll_details = {
            'type': 'attack_roll',
        ...
        if not self.is_interception_attack:
            ...
            if attacker_can_reroll or defender_can_reroll:
                ...
                # 控制器将保存 self.to_dict()，其中包含 raw_rolls
    ...
    def _resolve_chosen_effect(self, log, chosen_effect, result_packet=None):
        """
        (私有) 玩家或 AI 选择了溢出效果后，解析该效果。
        这可能会导致 'RESOLVED' 或 'AWAITING_EFFECT_REROLL'。
        """

        if result_packet:
            log.append(f"  > 自动选择效果: 【{chosen_effect}】")
        else:
            result_packet = self._create_empty_packet('penetration')
            log.append(
                f"> 玩家选择了【{'毁伤' if chosen_effect == 'devastating' else ('霰射' if chosen_effect == 'scattershot' else '顺劈')}】！")

            # 必须将初次掷骰的结果复制到新的包中，以便前端显示
            result_packet['dice_roll_details'] = self.initial_dice_roll_details.copy()

        target_part = self.defender_entity.get_part_by_name(self.target_part_name)
        if not target_part:
            log.append(f"  > [错误] 效果结算时找不到部件 '{self.target_part_name}'。")
            self.stage = 'RESOLVED'
            return log, self._create_empty_packet('invalid')
        ...
        log, secondary_roll_details, effect_packet_ext = self._calculate_effect_logic(
            ...
            skip_reroll_phase=False  # 效果掷骰 *可以* 被重投
        )
        ...
        if effect_packet_ext.get('status') == 'reroll_choice_required':
            self.stage = 'AWAITING_EFFECT_REROLL'  # 切换到效果重投
            result_packet['status'] = 'reroll_choice_required'

            # 存储效果重投所需的数据
            self.pending_effect_reroll_data = effect_packet_ext['pending_reroll_data']
            return log, result_packet
    ...
    def _calculate_effect_logic(self, log, attacker_entity, defender_entity, target_part, overflow_hits, overflow_crits,
        ...
        返回:
            tuple: (log, dice_roll_details_2, packet_extension)
                   其中 packet_extension 包含了状态变更和可能的中断。
        """
