...
def build_effects(*effects_list):
    """
    从效果库变量中构建最终的 effects 字典。
    它会合并所有 "logic" 键，并收集所有 "name" 文本。
    """
    final_effects = {}
    display_list = []
    ...
    for effect in effects_list:
        # 检查是否符合静止条件来触发射程加成
        if effect.get("logic", {}).get("is_static", False):
            if "static_range_bonus" in effect.get("logic", {}):
                display_list.append("【+2射程】")  # 或者根据需要修改
            if "yellow_dice_bonus" in effect.get("logic", {}):
                display_list.append("【+1黄骰子】")

        # 合并效果的逻辑
        final_effects.update(effect.get("logic", {}))

        # 如果有 "name" 属性，则添加到显示效果列表
        if "name" in effect:
...
# 每个效果现在包含 'logic' (供后端使用) 和 'name' (效果名称)
EFFECT_AP_1 = {
    "logic": {"armor_piercing": 1},
    "name": "【穿甲1】"
}
#EFFECT_STATIC_RANGE_2 = {
#    "logic": {"static_range_bonus": 2},
#    "name": "【【静止】+2射程】"
#}
...
# 静止状态效果：如果符合静止要求，增加射程
EFFECT_STATIC = {
    "logic": {"is_static": True},  # 表示机甲处于静止状态
    "name": "【静止】"
}

EFFECT_STATIC_RANGE_PLUS = {
    "logic": {"static_range_bonus": 2},  # 触发时增加射程
    "name": "【+2射程】"
}

# 结合静止与射程加成的效果，只有静止时触发射程加成
EFFECT_STATIC_RANGE_2 = {
    "logic": {
        "is_static": True,  # 必须处于静止状态
        "static_range_bonus": 2  # 才能触发射程加成
    },
    "name": "【静止 +2射程】"
}

# 为未来扩展：例如：静止状态下 +1 黄骰子效果
EFFECT_STATIC_PLUS_YELLOW_DICE = {
    "logic": {
        "is_static": True,  # 必须处于静止状态
        "yellow_dice_bonus": 1  # 触发 +1 黄骰子
    },
    "name": "【静止 +1黄骰子】"
}
...
EFFECT_TWO_HANDED_SNIPER = {
    ...
    "name": "【【双手】获得狙击】" # 【狙击】效果本身由后端逻辑处理，这里只做标记
}
# [新增] 毁伤效果
EFFECT_DEVASTATING = {
